...
- PCM 24kHz 16-bit mono LE, base64 in `input_audio_buffer.append`; `languages` ISO-639-1 array (not `language`); auto here `["de","en"]`, single code when Settings language is de/en. `turn_detection: null`; do not send `delay`. Hold-to-talk: one buffer, one commit on release (flush remainder → `input_audio_buffer.commit` → wait `.completed`). Periodic preview commits fragment sentences (each chunk ends with `.`/`,` and earlier chunks never rewrite). Overlay has no transcript box; hide on release, paste after `.completed`.
